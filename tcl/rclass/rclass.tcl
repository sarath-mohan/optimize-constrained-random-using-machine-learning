...
namespace eval ::rclass {
    ...
    set max_rand_sim_count 10;
  
    set ML_enabled 0;

    set fsm_opt_enable 0;

    fann create DNNa 2 1 1;

    fann create DNNb 2 1 1;
...
proc ::rclass::final_report {} {
    ...
    variable width
    set width [get top.width -radix decimal]
    puts "INFO STATUS : TCL : WIDTH OF COMPARATOR = ${width}"
...
proc ::rclass::eval_loop {} {
    ...
    variable stable_count
    variable out_vec

    variable TS_ready
    variable max_rand_sim_count
    variable ML_enabled
    variable fsm_opt_enable
    ...
    if { [string equal $status "EXISTING"] } {
        ...
        # print out timesout_vec and seed values used
        set ptime [senv time];
        ...
        if { [expr "$new_objective" > "$old_objective"] || [expr "$new_objective" >= ${max_objective}] } {
            ...
            puts "DEBUG stable_count == $stable_count";
            #set training_done [get top.rseed_interface.DNN_train_done]
            #puts "DEBUG training_done == $training_done";
            #if {$training_done == 1} {run_DNN}
            ...
            if { ($stable_count >= $max_rand_sim_count) && (($fsm_opt_enable==1) || ($ML_enabled>0))} {
            	#puts "DEBUG \"Generate Reset\" Function called";
                call top.rseed_interface.generate_reset();
                set stable_count 0;
                # puts "DEBUG stable_count == $stable_count";
                # TODO: Must be generalized by replacing "0" in TS_ready_0 with "[no_of_TS_required]-1"
                #set TS_ready [get top.dif.TS_ready_0 -radix decimal]
                #if {($ML_enabled == 1) && ($TS_ready == 1)} {eval_coeff}
	        set TS_ready [get top.dif.TS_ready -radix decimal]
                if {($ML_enabled == 1) && ($TS_ready == 1)} {eval_coeff}
	        if {($ML_enabled == 2) && ($TS_ready == 1)} {train_DNN}
	    }

            set last_seed [expr {int(rand()*1234+1)}]
            #???  4294967294 Why was this value chosen : Throws error "Integer value too large"  ???
...
proc ::rclass::eval_coeff {} {
    ...
    variable ctime
    set ctime [senv time]
    puts "######################### START eval_coeff"
    puts "INFO STATUS : TCL : Evaluating Coefficients for LR Model at time: $ctime"
...
proc ::rclass::train_DNN {} {
    variable ctime
    set ctime [senv time]
    puts "************************* START train_ANN"
    puts "INFO STATUS : TCL : TRAINING ANN at time: $ctime"
    variable a0
    variable a1
    variable b0
    variable b1
    ...
    set a0 [get top.dif.TS_a_0 -radix decimal]
    set a1 [get top.dif.TS_a_1 -radix decimal]
    set b0 [get top.dif.TS_b_0 -radix decimal]
    set b1 [get top.dif.TS_b_1 -radix decimal]
    set match0 [get top.dif.TS_match_0 -radix decimal]
    set match1 [get top.dif.TS_match_1 -radix decimal]
    
    puts "Training Sets:";
    puts "A0 = $a0; B0 = $b0; MATCH0 = $match0;";
    puts "A0 = $a1; B1 = $b1; MATCH0 = $match1;";
    
    # Train DNN
    #fann create DNNa 2 1 1
    DNNa function hidden linear
    DNNa function output linear
    eval "DNNa trainondata 500000 0 {{$a0} {$a1}} {{$match0} {$match1}}"
    
    #fann create DNNb 2 1 1
    DNNb function hidden linear
    DNNb function output linear
    eval "DNNb trainondata 500000 0 {{$b0} {$b1}} {{$match0} {$match1}}"

    # Force DNN Training Done to SV Environment
    force top.rseed_interface.DNN_train_done 1;
    puts "************************* END train_ANN"
...
proc ::rclass::run_DNN {} {
    ...
    set ctime [senv time]
    puts "************************* START run_DNN"
    puts "INFO STATUS : TCL : RUNNING DNN at time: $ctime"
    variable in;
    variable out;
    set in [get top.rseed_interface.in_b -radix decimal]
    set out [expr {round([eval "DNNb run {$in}"])}]
    puts "INFO_STATUS : TCL : Output Bin NOT Hit: $in; Predicted Input to Update Constraint Queue: $out";
    force top.rseed_interface.out_to_update_queue $out;
    puts "************************* END run_DNN"
}
...
proc ::rclass::main {} {
    ...
    set max_rand_sim_count [get top.rseed_interface.max_rand_sim_count]
    set ML_enabled [get top.rseed_interface.ml_enabled]
    set fsm_opt_enable [get top.rseed_interface.fsm_opt_enable]
